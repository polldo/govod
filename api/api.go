...
func APIMux(cfg APIConfig) http.Handler {
	...
	a.Handle(http.MethodPost, "/auth/logout", auth.HandleLogout(cfg.DB, cfg.Session))
	a.Handle(http.MethodGet, "/auth/oauth-login/{provider}", auth.HandleOauthLogin(cfg.DB, cfg.Session))
	a.Handle(http.MethodGet, "/auth/oauth-callback", auth.HandleOauthCallback(cfg.DB, cfg.Session))
	...
	a.Handle(http.MethodGet, "/mycourses", course.HandleListOwned(cfg.DB), authen)
